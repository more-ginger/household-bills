...
<div class="relative h-full w-full overflow-hidden" bind:clientWidth={width}>
	{#key step}
		<div
			...
			class={`absolute inset-0 flex items-center justify-center overflow-hidden ${FigureData['figure-steps'][step].title === 'population' ? 'p-4 md:p-10' : 'p-4'}`}
		>
			{#if FigureData['figure-steps'][step].title === 'population' || FigureData['figure-steps'][step].title === 'population-end'}
			...
			{:else}
				<div class="flex flex-wrap justify-center">
					...
					{#each FigureData['figure-steps'][step].urls as figure}
						<img
							class={`${FigureData['figure-steps'][step].css} w-auto max-w-full object-contain md:max-h-[80vh]`}
							src={resolve(figureUrl(FigureData['figure-steps'][step], figure))}
							alt=""
						/>
					{/each}
				</div>
			...
			{#if FigureData['figure-steps'][step]['has-footnote']}
				...
				>
					<img
						class={`${FigureData['figure-steps'][step].title === 'population' ? 'h-[2em] md:h-[4em] ' : 'h-[2em] md:h-[1.5em]'} mr-6 w-auto`}
